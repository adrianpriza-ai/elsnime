...
Elsnime is an Android anime streaming client with a vanilla HTML/CSS/JavaScript frontend in a WebView, native Java scrapers, and SQLite storage. No accounts, no telemetry, no tracking, and no intrusive permission requests.
...
- **Inline HLS Player**: Plyr with hls.js for adaptive HLS playback, vendored inline so the player boots offline.
- **Quality Switching**: Choose from available HLS renditions or let the player adapt to bandwidth; starting quality comes from Settings.
...
- **Home Page Overhaul**: Five scrollable rows for Continue Watching, Popular This Month, New Episodes, Up & Coming, and All-Time Popular.
- **Horizontal Scroll Layout**: Search results and trending section use horizontal scroll with snap navigation.
- **Follow & Notify System**: Full episode tracking with push notifications when followed anime release new episodes.
- **Library Hub**: Quick access to saved anime and followed series with episode counts.
...
- Full watch logs in SQLite; delete entries individually.
